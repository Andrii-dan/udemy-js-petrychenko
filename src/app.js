...
function isBudgetEnough(data) {
	const everyShopVolume = [];

	data.shops.forEach((el) => {
		everyShopVolume.push(el.width * el.length * data.height);
	});

	const mallVolume = everyShopVolume.reduce((a, b) => a + b);
	const mallHeatingPrice = mallVolume * data.moneyPer1m3;

	if (mallHeatingPrice <= data.budget) {
